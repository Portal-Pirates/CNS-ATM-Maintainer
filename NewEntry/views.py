...
def NewEntry(request, id=None, report_type=None):
    if request.method == 'POST':
        ...
        if Stations.objects.filter(station_name__iexact=st_name).exists():
            ...
            if eq_obj.equipment_name.upper() == 'UPS': #workking
                ...
                if report_type.upper() == 'MONTHLY':
                    ...
                    messages.success(request, "Saved Successfully...")
                    return redirect('/')
            if eq_obj.equipment_name.upper() == 'COMSOFT': #Working
                if report_type == 'DAILY':
            ...
            if eq_obj.equipment_name.upper() == 'GLIDPATH':
                if report_type.upper() == 'DAILY':
                    ...
                    glidpath_obj = Glid_Path(smps=smps,
                    main=main,
                    txtoair=txtoair,
                    clearnance=clearnance,
                    course=course,
                    standby=standby,
                    maint=maint,
                    batt=batt,
                    Disagr=Disagr,
                    param=param,
                    remote_status_indication=remote_status_indication,
                    status_critical_area=status_critical_area,
                    status_near_monitor=status_near_monitor,
                    verify_equipment_on_UPS=verify_equipment_on_UPS,
                    main_supply_frequency=main_supply_frequency,
                    main_supply_volt=main_supply_volt,
                    status_obstruction_light=status_obstruction_light,
                    Status_airconditioners=Status_airconditioners,
                    General_cleanliness_room=General_cleanliness_room,
            ...
            if eq_obj.equipment_name.upper() == 'DATISTERMA':
                if report_type.upper() == 'DAILY':
                    ndb_obj = NDB(status=status, equipment_name=eq_obj.equipment_name,
                                  modal_number=eq_obj.modal_number, Make=eq_obj.company,
                                  Airport_Location=location, station_name=st_obj,
                                  created_by=user_profile,
                                  report_type=report_type.lower())
            ...
            if eq_obj.equipment_name.upper() == 'DVTR':
                if report_type.upper() == 'DAILY':
                    ...
                    ndb_obj = NDB(room_temp=room_temp,AC_main_volt=AC_main_volt,
                ...
                if report_type.upper() == 'MONTHLY':
                    ...
                    ndb_obj = NDB(cleaning_PC_dust=cleaning_PC_dust,
