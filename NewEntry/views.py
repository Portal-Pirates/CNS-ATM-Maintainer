    ...
    try:
        if request.method == "GET":
            if request.user.is_authenticated:
                ...
                return redirect("login") #It should be updated with login Url
    ...
    if request.method == 'POST':
        try:
            ...
            if Equipments.objects.filter(serial_number=serial_number).exists():
                ...
                obj = Equipments.objects.filter(serial_number=serial_number).first()
                ...
                for letter in temp:
                    eq_name += letter.upper()
                newobj = Equipments(equipment_name=eq_name,station_name=station_name ,serial_number=serial_number, modal_number=modal_number, equipment_type=equipment_type, company=company)
                newobj.save()
                try:
                    if FaultTable.objects.filter(Equipment_Name= eq_name).exists():
                        remarks = FaultTable.objects.filter(Equipment_Name__iexact=eq_name).first()
                        if remarks.Remarks=="Maintainable":
                            newobj.is_maintain = True
                            newobj.is_replace = False
                            newobj.is_both = False
                            days_obj = PredictedTable.objects.filter(Equipment_Name__iexact=eq_name).first()
                            number_of_days = days_obj.prediction
                            start_date = newobj.created
                            print(start_date)
                            start_date = start_date.strftime("%d")+"/"+ str(start_date.month)+"/"+str(start_date.year)[2:]
                            date_1 = datetime.datetime.strptime(start_date, "%m/%d/%y")
                            end_date = date_1 + datetime.timedelta(days=number_of_days)
                            print(end_date)
                            newobj.maintainence_date = end_date
                        if remarks.Remarks=="Replacable":
                            newobj.is_maintain = False
                            newobj.is_replace = True
                            newobj.is_both = False
                            days_obj = PredictedTable.objects.filter(Equipment_Name__iexact=eq_name).first()
                            number_of_days = days_obj.prediction
                            start_date = newobj.created
                            ...
                            print(end_date)
                            newobj.maintainence_date = end_date
                            newobj.replacement_date = end_date
                            newobj.save() 
...
def predictdata(request):
    try:
        ...
        if username.Profile.is_manager:
            x = Profile.objects.filter(user = request.user).first()
            today = date.today() 
            equipment_query = Equipments.objects.filter(station_name = x.working_location)
            if equipment_query:
                ...
                replacement = ""
                maintainence = ""
                for i in equipment_query_for_replacement:
                    replacement+= i.equipment_name+"\t"+i.modal_number+"\t"+i.replacement_date+"\n"
                for j in equipment_query_for_maintainence:
                    maintainence+=j.equipment_name+"\t"+j.modal_number+"\t"+j.maintainence_date+"\n"
                message = "Replacement Data"+"\n"+replacement+"\n"+"Maintainence data"+"\n"+maintainence    
                email1 = mail.EmailMessage(
                    'Alerts',
                    message,
                'ac7908840@gmail.com',
                    ['mayankirar@gmail.com'],
                connection=connection,
                )
                email1.send()
    except Exception as e:
        messages.info(request,"Succussfully send!")
            ...
            if Stations.objects.filter(station_name__iexact=st_name).exists():
                ...
                if eq_obj.equipment_name.upper() == 'UPS':
                    ...
                    if report_type.upper() == 'WEEKLY':
                        ...
                        upsobj = UPS(remarks=remarks,status=status, Earthing_codition=Earthing_codition,Neutral_load=Neutral_load, UPS_load_percent=UPS_load_percent, UPS_condition=UPS_condition, Battery_condition=Battery_condition, check_battery_terminal=check_battery_terminal, Power_supply_terminal=Power_supply_terminal,equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                        upsobj.save()
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    if report_type.upper() == 'MONTHLY':
                        ...
                        upsobj = UPS(remarks=remarks,status=status, Netuno_Server_restart=Netuno_Server_restart, DAT_drive_cleaning=DAT_drive_cleaning, Physica_check_antenna_cables=Physica_check_antenna_cables, Control_Panel_setup=Control_Panel_setup, Rx_module_status=Rx_module_status, Physicaly_clean_check=Physicaly_clean_check, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                        upsobj.save()
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    upsobj = UPS(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                    upsobj.save()
                    messages.success(request, "Saved Successfully🥳...")
                    return redirect('/')
                if eq_obj.equipment_name.upper() == 'COMSOFT':
                    ...
                    if report_type.upper() == 'MONTHLY':
                        ...
                        comsoft_obj = COMSOFT(remarks=remarks,status=status,UTC_synchr=UTC_synchr,overall_build_tst_result=overall_build_tst_result,Gnd_station_staus=Gnd_station_staus,Decoder_test=Decoder_test,Test_transmission_loss=Test_transmission_loss,Rx_sensitivity=Rx_sensitivity,End_to_End_test=End_to_End_test,Signal_strength_RF_Site=Signal_strength_RF_Site,DSP_board_tmep=DSP_board_tmep,DSP_Board_volt=DSP_Board_volt,FPGA_volt=FPGA_volt,Gps_volt=Gps_volt,Intermediate_volt=Intermediate_volt,Gps_synch=Gps_synch,Cpu_load=Cpu_load,Reciever=Reciever,FPGA=FPGA,voltage_sensing=voltage_sensing,GPS=GPS,general_result=general_result,Gps_altitude=Gps_altitude,GPS_longitude=GPS_longitude, Gps_latitude=Gps_latitude, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                        comsoft_obj.save()
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    comsoft_obj = COMSOFT(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                    comsoft_obj.save()
                    messages.success(request, "Saved Successfully🥳...")
                    return redirect('/')
                if eq_obj.equipment_name.upper() == 'GLIDPATH':
                    if report_type.upper() == 'DAILY':
                        ...
                        Status_airconditioners = int(request.POST['Status_airconditioners'])
                        status_obstruction_light= int(request.POST['status_obstruction_light'])
                        print(status_obstruction_light)
                        main_supply_volt= int(request.POST['main_supply_volt'])
                        main_supply_frequency= int(request.POST['main_supply_frequency'])
                    ...
                    if report_type.upper() == 'MONTHLY':
                        ...
                        glidpath_obj = Glid_Path(remarks=remarks,status_rc_lines=status_rc_lines,Envionment=Envionment,grading_of_critical_are=grading_of_critical_are,all_function_remote=all_function_remote,moitor_sys_oper=moitor_sys_oper,total_time=total_time,reduciton_power=reduciton_power,clearance_signal=clearance_signal,change_in_displacement_sens=change_in_displacement_sens,Path_augle=Path_augle, status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                        glidpath_obj.save()
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    if report_type.upper() == 'YEARLY':
                        ...
                        glidpath_obj = Glid_Path(remarks=remarks,phase_of_modulation_system=phase_of_modulation_system,carrier_modulation_freq=carrier_modulation_freq,unwanted_modulation=unwanted_modulation,Frequency=Frequency,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                        glidpath_obj.save()
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                ...
                if eq_obj.equipment_name.upper() == 'VCSSYSTEM':
                    if report_type.upper() == 'DAILY':
                        ...
                        vcs_obj = VCS_System(remarks=remarks, Overall_perform=Overall_perform,
                                    system_A=system_A,
                    ...
                    if report_type.upper() == 'WEEKLY':
                        ...
                        vcs_obj =  VCS_System(remarks=remarks, DMC01D5=DMC01D5,
                                    D2=D2,
                                    D3=D3,
                                    D4=D4,
                                    PDE_4601=PDE_4601,
                        ...
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    vcs_obj =  VCS_System(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                    vcs_obj.save()
                    messages.success(request, "Saved Successfully🥳...")
                    return redirect('/')
                ...
                if eq_obj.equipment_name.upper() == 'LOCALIZER':
                    if report_type.upper() == 'DAILY':
                        ...
                        main_supply_volt = int(request.POST['main_supply_volt'])
                        main_supply_frequency = int(request.POST['main_supply_frequency'])
                        localizer_obj = Localizer(remarks=remarks,status_obstruction_light=status_obstruction_light,
                    ...
                    if report_type.upper() == 'WEEKLY':
                        ...
                        localizer_obj = Localizer(remarks=remarks,System_time_synch=System_time_synch,
                    ...
                    if report_type.upper() == 'MONTHLY':
                        ...
                        localizer_obj = Localizer(remarks=remarks,Equ_rack_earth_resis=Equ_rack_earth_resis,
                    ...
                    if report_type.upper() == 'YEARLY':
                        Frequency= int(request.POST['Frequency'])
                        carrier_modulation_freq= int(request.POST['carrier_modulation_freq'])
                        carrier_module_har_content= int(request.POST['carrier_module_har_content'])
                        Carrier_modulation_harm= int(request.POST['Carrier_modulation_harm'])
                        Unwanted_modulation= request.POST['Unwanted_modulation']
                        CSB_in_watt=int(request.POST['CSB_in_watt'])
                        SBO= int(request.POST['SBO'])
                        phase_CORR= int(request.POST['phase_CORR'])
                        LF_phase_check=int( request.POST['LF_phase_check'])
                        RF_phase_check=int( request.POST['RF_phase_check'])
                        DDM_check_COU=int( request.POST['DDM_check_COU'])
                        SDM_check_COU= int(request.POST['SDM_check_COU'])
                        Course_ident_Mod= int(request.POST['Course_ident_Mod'])
                        Cou_Trans_out_pow= int(request.POST['Cou_Trans_out_pow'])
                        CLR_Tranns_out_pow= int(request.POST['CLR_Tranns_out_pow'])
                        Monitor_delay=int( request.POST['Monitor_delay'])
                        localizer_obj= Localizer( 
                            Monitor_delay=Monitor_delay,
                            CLR_Tranns_out_pow=CLR_Tranns_out_pow,
                            Cou_Trans_out_pow=Cou_Trans_out_pow,
                            Course_ident_Mod=Course_ident_Mod,
                            SDM_check_COU=SDM_check_COU,
                            DDM_check_COU=DDM_check_COU,
                            RF_phase_check=RF_phase_check,
                            LF_phase_check=LF_phase_check,
                            phase_CORR=phase_CORR,
                            SBO=SBO,
                            CSB_in_watt=CSB_in_watt,
                            Unwanted_modulation=Unwanted_modulation,
                            Carrier_modulation_harm=Carrier_modulation_harm,
                            carrier_module_har_content=carrier_module_har_content,
                            carrier_modulation_freq=carrier_modulation_freq,Frequency=Frequency,remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                        localizer_obj.save()
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                ...
                if eq_obj.equipment_name.upper() == 'DVOR':
                    if report_type.upper() == 'DAILY':
                        ...
                        Dvor_obj = DVOR(remarks=remarks,status_air_conditioners=status_air_conditioners,
                    ...
                    if report_type.upper() == 'WEEKLY':
                        ...
                        USB_frequency = int(request.POST['USB_frequency'])
                        ...
                        battery_current =int( request.POST['battery_current'])
                        Dvor_obj = DVOR(remarks=remarks,Carrier_pow=Carrier_pow,
                                        Am_1020hz=Am_1020hz,
                    ...
                    if report_type.upper() == 'MONTHLY':
                        ...
                        Dvor_obj = DVOR(remarks=remarks,Eq_earth_rack_resistance=Eq_earth_rack_resistance,
                ...
                if eq_obj.equipment_name.upper() == 'NDB':
                    if report_type.upper() == 'DAILY':
                        ...
                        ndb_obj=NDB(remarks=remarks,room_temp=room_temp,
                    ...
                    if report_type.upper() == 'WEEKLY':
                        ...
                        ndb_obj = NDB(remarks=remarks,main_pow_supp_terminal=main_pow_supp_terminal,
                    ...
                    if report_type.upper() == 'MONTHLY':
                        ...
                        ndb_obj = NDB(remarks=remarks,Carry_out_pow_measur=Carry_out_pow_measur,
                    ...
                    if report_type.upper=='YEARLY':
                        ...
                        ndb_obj = NDB(remarks=remarks,check_carrier_freq=check_carrier_freq,
                ...
                if eq_obj.equipment_name.upper() == 'DATISTERMA':
                    if report_type.upper() == 'DAILY':
                        ndb_obj = Datis_Terma(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name,
                                    modal_number=eq_obj.modal_number, Make=eq_obj.company,
                                    Airport_Location=location, station_name=st_obj,
                                    created_by=user_profile,
                                    report_type=report_type.lower())
                        ...
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    d_obj = Datis_Terma(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                    d_obj.save()
                    messages.success(request, "Saved Successfully🥳...")
                    return redirect('/')
                if eq_obj.equipment_name.upper() == 'DVTR':
                    if report_type.upper() == 'DAILY':
                        ...
                        ndb_obj = DVTR(remarks=remarks,room_temp=room_temp,AC_main_volt=AC_main_volt,
                    ...
                    if report_type.upper() == 'MONTHLY':
                        ...
                        ndb_obj = DVTR(remarks=remarks,cleaning_PC_dust=cleaning_PC_dust,
                        ...
                        messages.success(request, "Saved Successfully🥳...")
                        return redirect('/')
                    ndb_obj = DVTR(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                    ndb_obj.save()
                    messages.success(request, "Saved Successfully🥳...")
                    return redirect('/')
                otherEq_obj = OtherEquipmentsReport(remarks=remarks,status=status, equipment_name=eq_obj.equipment_name, modal_number=eq_obj.modal_number, Make=eq_obj.company, Airport_Location=location, station_name=st_obj, created_by=user_profile, report_type=report_type.lower())
                otherEq_obj.save()
                messages.success(request, "Saved Successfully🥳...")
                return redirect('/')
                ...
                location_obj = Airports.objects.filter(Airport_Location__iexact=location).first()
...
# After Qr code process
def chooseReportAfterQr(request, eq_id=None, report_type=None, st_name = None):
    try:
        ...
        return render(request, "chooseTypeAfterQr.html", context)
    except Exception as e:
        print(e)
        messages.info(request, "Something went Wrong!! Please try Again...")
        return redirect('/')
...
def NewEntryAfterQr(request, eq_id=None, report_type=None, st_name = None):
    if request.method == 'GET':
        try:
            print(eq_id)
            eq_obj = Equipments.objects.filter(id=eq_id).first()
            report_type = report_type.upper()
            eq_name = eq_obj.equipment_name.upper()
            st_name = st_name.upper()
            context = {
                    'st_name': st_name,
                    eq_name : True,
    ...
    if request.method == 'POST':
        try:
            ...
            st_obj = Stations.objects.filter(station_name__iexact=st_name).first()
            location = st_obj.location.Airport_Location
            remarks  = request.POST['remarks'] 
            status  = int(request.POST['status'])
            if eq_obj.equipment_name.upper() == 'UPS':
                if report_type.upper() == 'DAILY':
